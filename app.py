...
# Define the Pydeck 3D Canvas Layer Framework (FIXED COLOR CODES)
...
arc_layer = pdk.Layer(
    ...
    get_source_color=[0, 255, 128, 200], get_target_color=[255, 0, 0, 200], get_width=5
)

column_layer = pdk.Layer(
    "ColumnLayer", data=map_data, get_position="[to_lon, to_lat]", get_elevation="threat_level",
    elevation_scale=1, radius=50000, get_fill_color=[255, 0, 0, 150], pickable=True, auto_highlight=True
)
...
chart_data = pd.DataFrame({
    'Hypothesis Framework': ['Weather Variance (H1)', 'Internal Operational Bottleneck (H2)', 'Exogenous Asymmetric Anomaly (H3)'],
